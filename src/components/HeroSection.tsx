...
const HeroSection = () => {
  ...
  return (
    <section className="relative h-[calc(100vh-64px)] overflow-hidden flex items-center bg-white">
      {/* Image Container with Rounded Edges and Overlay for Contrast */}
      <div className="absolute inset-0 z-0 flex items-center justify-center">
        <div className="relative w-[calc(100%-40px)] h-[calc(100%-40px)] mx-auto my-auto rounded-3xl overflow-hidden shadow-lg">
      ...
      <div className="relative z-10 w-full">
        <div className="w-[calc(100%-120px)] max-w-[1200px] mx-auto px-4 md:px-6 lg:px-8">
          <div ref={containerRef} className="text-center">
            {/* Company Badge */}
            <div ref={badgeRef} className="inline-flex items-center px-4 py-2 rounded-full bg-white/10 text-white text-sm font-medium mb-8">
              <HugeiconsIcon icon={CloudIcon} size={16} className="mr-2" />
              Born in the Canary Islands, Serving the World
            </div>
            
            {/* Main Headline */}
            <h1 ref={titleRef} className="text-3xl md:text-4xl lg:text-5xl font-medium tracking-tight text-white mb-6 leading-snug font-display">
            ...
            {/* Subheadline */}
            <p ref={subtitleRef} className="text-base md:text-lg text-white/90 mb-6 max-w-3xl mx-auto leading-relaxed">
            ...
            <div ref={buttonsRef} className="flex flex-col sm:flex-row gap-3 justify-center">
              <button className="bg-brand-primary hover:bg-brand-primary-700 text-white px-6 py-3 rounded-full text-base font-semibold transition-colors duration-200 flex items-center justify-center group">
              ...
              <button className="border-2 border-white text-white hover:bg-white/10 px-6 py-3 rounded-full text-base font-semibold transition-colors duration-200 flex items-center justify-center">
                <HugeiconsIcon icon={CloudIcon} size={16} className="mr-2" />
                View Our Solutions
              </button>
            </div>
